...
fn main() {
    let contents = fs::read_to_string("src/test.txt").unwrap();
    ...
    for (row, line) in lines.clone().enumerate() {
        for num in re_num.find_iter(line) {
            let num_col_start: usize = num.start();
            let num_col_end: usize = num.end();
            let is_num_start_part: bool = is_symbol_surrounding(&row, num_col_start, &grid, &re_sym);
            let is_num_end_part: bool = is_symbol_surrounding(&row, num_col_end, &grid, &re_sym);
